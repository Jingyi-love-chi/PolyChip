...
extern "C"
  ...
  if (size0 != 127 || size1 != 127 || stride0 != 127 || stride1 != 1) {
    printf("FAILED: tile matmul 127x17 unexpected memref shape "
  ...
  const int32_t expected = 0x41880000;
  for (int i = 0; i < 127; ++i) {
    for (int j = 0; j < 127; ++j) {
      int32_t got = c[i * stride0 + j * stride1];
      if (got != expected) {
        printf("FAILED: tile matmul 127x17 c[%d][%d] "
  ...
  printf("PASSED: tile matmul 127x17 @ 17x127\n");
}
